...
<script>
    ...
    export default {
        ...
        methods:{
         ...
         // Vue2.0 使用 mounted 替换 ready，但使用 mounted 并不能保证钩子函数中的 this.$el 在 document 中，为此还应该引入 Vue.nextTick/vm.$nextTick
        mounted() {
            this.$nextTick(() => { // 代码保证 this.$el 在 document 中
                this._initScroll();
                this._initPics();   // mounted 此刻的seller还是空对象，所以要使用 watch检测seller变化，或者使用 updated() 钩子函数，但尽量使用watcher和计算属性
            });
